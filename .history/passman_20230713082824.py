...
class Ui_MainWindow(object):
    ...
    def submit(self):
        ...
        if has_number == True and has_uppercase_char == True and has_lowercase_char == True and has_special_char == True and has_length == True:
            ...
            while i < 4:
                ...
                keyword += str(random.choice(special_characters))
            ...
            msgBox.setIcon(QMessageBox.Information)
            msgBox.setWindowTitle("Passman Password Manager")
            msgBox.setText("Your passwords didnt meet the standards.")
            s = "Your password should meet the conditions mentioned below \n 1. At least 1 uppercase letter \n 2. At least o lowercase letter \n 3. At least one number \n 4. At least one special character"
            msgBox.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
            msgBox.setInformativeText(f"Password Suggestion: {str(suggest)}")
    ...
    def search(self):
        ...
        if passcode == 'root':
            ...
            for row in rows:
                print(row)
            self.outputBox.setText(f" Username :{str(rows[-1][0])} Password :{str(rows[-1][1])}")
            # self.outputBox.setText(" Username \t Password \n\n "+str(rows[-1][0])+" \t "+str(rows[-1][1]))
            conn.commit()
            conn.close()
        else:
            print('wrong')
